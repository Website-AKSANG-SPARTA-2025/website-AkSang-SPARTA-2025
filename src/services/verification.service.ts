...
type VerificationParticipant = Pick<
  Participant,
...
async function synchronizeVerifiedParticipant(
...
export async function resendVerification(input: {
  ...
  if (!participant)
    throw new ApplicationError("NOT_FOUND", 404, "Participant not found");
  if (input.purpose === "ATTENDANCE") {
    if (!participant.attendance)
      throw new ApplicationError("NOT_FOUND", 404, "Attendance not found");
