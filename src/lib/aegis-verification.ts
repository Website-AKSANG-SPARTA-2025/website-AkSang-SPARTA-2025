...
export async function sendAegisVerification(
  email: string,
...
export async function getAegisVerificationStatus(
  email: string,
): Promise<AegisStatusResult> {
  ...
  try {
    ...
    if (body.status === "not_verified") {
      ...
      const expiresAt =
        body.linkExpiresAt === undefined
          ? undefined
          : timestamp(body, "linkExpiresAt");
      if (
        registeredAt &&
        typeof body.linkActive === "boolean" &&
        expiresAt !== null
      ) {
        return {
          status: "not_verified",
