...
describe("BE-02 request schemas", () => {
  ...
  it("accepts attendance confirmation data without forged identity", () => {
    expect(
      confirmAttendanceSchema.safeParse({ attendeeType: "PUBLIC" }).success,
    ).toBe(true);
    expect(
  ...
  it.each([
    ...
    [
      "invalid phone",
      {
        ...
        email: "grace@example.com",
        competitionPath: "CP",
        phoneNumber: "0812-345",
      },
    ],
  ])("rejects workshop enrollment with %s", (_case, payload) => {
    expect(createWorkshopEnrollmentSchema.safeParse(payload).success).toBe(
      false,
    );
  });
  ...
  it("accepts only valid resend identity and purpose", () => {
    ...
    expect(
      resendVerificationSchema.safeParse({ email: "invalid", purpose: "OTHER" })
        .success,
    ).toBe(false);
  });
  ...
  it.each([
    ...
    [
      "unsupported path",
      { competitionPath: "WEB", phoneNumber: "+62812345678" },
    ],
  ...
  it("accepts only an approved submission path", () => {
    expect(submissionSchema.safeParse({ competitionPath: "CP" }).success).toBe(
      true,
    );
    expect(submissionSchema.safeParse({ competitionPath: "WEB" }).success).toBe(
      false,
    );
  });
});
...
describe("BE-02 JSON request parsing", () => {
  it("parses and validates a JSON request body", async () => {
    ...
    await expect(
      parseJsonBody(request, createAttendanceSchema),
    ).resolves.toEqual({
      name: "Ada Lovelace",
      email: "ada@example.com",
      attendeeType: "PUBLIC",
