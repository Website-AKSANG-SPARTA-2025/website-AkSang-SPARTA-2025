...
beforeEach(() => {
  ...
  boundaries.setParticipantSessionCookie.mockImplementation(
...
afterEach(() => {
  ...
  if (originalDevAuthSecret === undefined)
    delete process.env.DEV_AUTH_TEST_SECRET;
...
describe("development session route", () => {
  ...
  it("sets the local session cookie only for a verified participant", async () => {
    ...
    expect(boundaries.findUnique).toHaveBeenCalledWith({
      where: { email: "ada@example.com" },
    });
    expect(boundaries.setParticipantSessionCookie).toHaveBeenCalledWith(
      expect.any(Response),
      "participant-1",
    );
  });
