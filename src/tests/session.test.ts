...
describe("participant sessions", () => {
  it("signs a payload containing only participantId and exp", async () => {
    ...
    await expect(
      readParticipantSession(requestWithToken(token)),
    ).resolves.toEqual(decoded);
    await expect(
      readParticipantSession(requestWithToken(`${token}x`)),
    ).resolves.toBeNull();
  });
  ...
  it("rejects expired and malformed session values", async () => {
    ...
    await expect(
      readParticipantSession(new Request("https://app.example.test")),
    ).resolves.toBeNull();
    await expect(
      readParticipantSession(
        new Request("https://app.example.test", {
          headers: { Cookie: "participant_session=not-a-token" },
        }),
      ),
    ).resolves.toBeNull();
  });
  ...
  it("sets and clears the approved cookie attributes", async () => {
    const localResponse = await setParticipantSessionCookie(
      new Response(),
      "participant-1",
    );
    ...
    const productionResponse = await setParticipantSessionCookie(
      new Response(),
      "participant-1",
    );
