...
describe("Aegis verification routes", () => {
  it("resends through Aegis and never exposes an email link or token", async () => {
    ...
    const response = await resendRoute(
      resendRequest({ email: "ada@example.com", purpose: "ATTENDANCE" }),
    );
  ...
  it("preserves resend cooldown and upstream error details", async () => {
    boundaries.resendVerification.mockRejectedValueOnce(
      new ApplicationError("RATE_LIMITED", 429, "Please wait", {
        retryAfter: 41,
      }),
    );
    const cooldown = await resendRoute(
      resendRequest({ email: "ada@example.com", purpose: "ATTENDANCE" }),
    );
    ...
    const tooMany = await resendRoute(
      resendRequest({ email: "ada@example.com", purpose: "ATTENDANCE" }),
    );
    ...
    const unavailable = await resendRoute(
      resendRequest({ email: "ada@example.com", purpose: "ATTENDANCE" }),
    );
  ...
  it("checks status for the signed-session participant instead of an arbitrary query email", async () => {
    ...
    const response = await statusRoute(
      new Request(
        "https://app.example.test/api/verifications/status?email=other@example.com",
      ),
    );
  ...
  it("returns status variants and rejects callers without a valid session", async () => {
    ...
    const verified = await statusRoute(
      new Request("https://app.example.test/api/verifications/status"),
    );
    ...
    const unknown = await statusRoute(
      new Request("https://app.example.test/api/verifications/status"),
    );
    ...
    const unauthorized = await statusRoute(
      new Request("https://app.example.test/api/verifications/status"),
    );
