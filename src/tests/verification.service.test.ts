  ...
  const prisma = {
    participant: {
      ...
      async updateMany({
        ...
        const participant = [...participants.values()].find(
          (record) =>
            record.id === where.id &&
            record.emailVerifiedAt === where.emailVerifiedAt,
        );
    ...
    attendance: {
      async updateMany({
        ...
        if (!attendance || attendance.status !== where.status)
          return { count: 0 };
    ...
    workshopRegistration: {
      async updateMany({
        ...
        if (!registration || registration.status !== where.status)
          return { count: 0 };
        registrations.set(registration.participantId, {
          ...registration,
          ...data,
        });
        return { count: 1 };
