...
vi.mock("../services/attendance.service", () => ({
  createAttendance: boundaries.createAttendance,
}));
vi.mock("../services/workshop.service", () => ({
  enrollWorkshop: boundaries.enrollWorkshop,
}));
...
describe("public entry routes", () => {
  ...
  it("creates workshop enrollment and sends through Aegis without exposing a token", async () => {
    ...
    const response = await enrollWorkshopRoute(
      jsonRequest(
        "https://app.example.test/api/workshops/enroll",
        workshopInput,
      ),
    );
    const body = await response.json();
