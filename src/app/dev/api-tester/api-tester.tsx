...
const inputClassName =
  "mt-1 w-full rounded border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900";
const buttonClassName =
  "rounded bg-zinc-900 px-3 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50";
...
export default function ApiTester() {
  ...
  const [resendPurpose, setResendPurpose] =
    useState<VerificationPurpose>("ATTENDANCE");
    ...
    <main className="mx-auto w-full max-w-6xl space-y-6 p-6 text-zinc-900">
      <header className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-zinc-600">
          Each button sends a live same-origin request. Use test email
          addresses: attendance, enrollment, and resend can ask Aegis to send
          email.
        </p>
      </header>

      <section className="rounded border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">
        <strong>Session testing:</strong> Aegis verification does not issue a
        browser session in this backend. The development form below creates a
        local session only for an already verified participant and is
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="space-y-4 rounded border border-zinc-200 p-4">
          ...
          <label className="block text-sm font-medium">
            ...
            <input
              ...
              type="email"
              value={developmentSession.email}
              onChange={(event) =>
          ...
          <label className="block text-sm font-medium">
            ...
            <input
              ...
              type="password"
              autoComplete="off"
              value={developmentSession.secret}
              onChange={(event) =>
                setDevelopmentSession({
                  ...developmentSession,
          ...
          <p className="text-sm text-zinc-600">
            Use only on a trusted local network. The secret is not stored or
            returned.
          </p>
          <button
            className={buttonClassName}
            disabled={busy || !developmentSession.secret}
            onClick={() =>
              void jsonRequest(
          ...
          >
            {pending === "POST /api/dev/auth/session"
              ? "Creatingâ€¦"
              : "POST /api/dev/auth/session"}
          </button>
        </section>
        ...
        <section className="space-y-4 rounded border border-zinc-200 p-4">
          ...
          <label className="block text-sm font-medium">
            ...
            <input
              ...
              onChange={(event) =>
                setAttendance({ ...attendance, email: event.target.value })
              }
            />
          </label>
          <label className="block text-sm font-medium">
            ...
            <select
              className={inputClassName}
              value={attendance.attendeeType}
              onChange={(event) =>
                setAttendance({
                  ...attendance,
                  attendeeType: event.target.value as AttendeeType,
                })
              }
            >
          ...
          >
            {pending === "POST /api/attendances/confirm"
              ? "Sending…"
              : "POST /api/attendances/confirm (session)"}
          </button>
        </section>
        ...
        <section className="space-y-4 rounded border border-zinc-200 p-4">
          <div>
            ...
            <p className="text-sm text-zinc-600">
              Resend is public; status is session-bound.
            </p>
          </div>
          <label className="block text-sm font-medium">
            Resend email
          ...
          <label className="block text-sm font-medium">
            ...
            <select
              ...
              value={resendPurpose}
              onChange={(event) =>
                setResendPurpose(event.target.value as VerificationPurpose)
              }
            >
          ...
          <button
            ...
            onClick={() =>
              void jsonRequest(
                ...
                { email: resendEmail, purpose: resendPurpose },
                true,
              )
            }
          >
            {resendCooldown > 0
              ...
              : pending === "POST /api/verifications/resend"
                ? "Sending…"
                : "POST /api/verifications/resend"}
          </button>
          <button
          ...
          >
            {pending === "GET /api/verifications/status"
              ? "Checking…"
              : "GET /api/verifications/status (session)"}
          </button>
        </section>
        ...
        <section className="space-y-4 rounded border border-zinc-200 p-4">
          <div>
            ...
            <p className="text-sm text-zinc-600">
              Enrollment is public; registration needs a verified session.
            </p>
          </div>
          <label className="block text-sm font-medium">
            Name
          ...
          <label className="block text-sm font-medium">
            ...
            <input
              ...
              onChange={(event) =>
                setWorkshop({ ...workshop, email: event.target.value })
              }
            />
          </label>
          <label className="block text-sm font-medium">
            ...
            <select
              className={inputClassName}
              value={workshop.competitionPath}
              onChange={(event) =>
                setWorkshop({
                  ...workshop,
                  competitionPath: event.target.value as CompetitionPath,
                })
              }
            >
              {paths.map((path) => (
          ...
          <label className="block text-sm font-medium">
            ...
            <input
              ...
              onChange={(event) =>
                setWorkshop({ ...workshop, phoneNumber: event.target.value })
              }
            />
          </label>
          <label className="block text-sm font-medium">
            ...
            <input
              ...
              value={workshop.nim}
              onChange={(event) =>
                setWorkshop({ ...workshop, nim: event.target.value })
              }
            />
          </label>
          <button
          ...
          >
            {pending === "POST /api/workshops/enroll"
              ? "Sending…"
              : "POST /api/workshops/enroll"}
          </button>
          <button
            ...
            onClick={() =>
              void jsonRequest(
                ...
                workshopRegistrationBody,
              )
            }
          >
            {pending === "POST /api/workshops/register"
              ? "Sending…"
              : "POST /api/workshops/register (session)"}
          </button>
          <a
            className={`${buttonClassName} inline-block`}
            href="/api/workshops/invitation"
          >
        ...
        <section className="space-y-4 rounded border border-zinc-200 p-4">
          <div>
            ...
            <p className="text-sm text-zinc-600">
              Requires a verified session and active workshop registration.
            </p>
          </div>
          <label className="block text-sm font-medium">
            Competition path
            <select
              ...
              onChange={(event) =>
                setSubmissionPath(event.target.value as CompetitionPath)
              }
            >
              {paths.map((path) => (
                <option key={path} value={path}>
            ...
            onClick={() => {
              if (!submissionFile) {
                setResult({
                  ...
                  status: null,
                  ok: false,
                  body: { message: "Choose a PDF file first" },
                });
                return;
              }
              ...
              void runRequest("POST /api/submissions", "/api/submissions", {
                method: "POST",
                body: form,
              });
            }}
      ...
      <section className="rounded border border-zinc-200 p-4">
        ...
        <pre
          aria-live="polite"
          className="mt-3 overflow-x-auto rounded bg-zinc-950 p-4 text-sm text-zinc-100"
        >
          {result
