...
async function sessionKey(): Promise<CryptoKey> {
  ...
  if (secret.byteLength < 32) throw new Error("SESSION_SECRET must be at least 32 bytes");
...
export async function readParticipantSession(request: Request): Promise<ParticipantSession | null> {
  ...
  try {
    ...
    if (!isParticipantSession(parsed) || parsed.exp <= Math.floor(Date.now() / 1_000)) return null;
    return parsed;
  } catch {
    return null;
  }
...
export function clearParticipantSessionCookie(response: Response): Response {
  response.headers.append("Set-Cookie", `${cookieName}=; ${cookieAttributes(0)}`);
  return response;
}
