...
function invalidPdf(): ApplicationError {
  return new ApplicationError("VALIDATION_ERROR", 400, "Only PDF files are allowed");
}

function displayFileName(value: string): string {
  return value.replace(/[\\/\u0000-\u001F\u007F]/g, "_").trim() || "submission.pdf";
}

export function validatePdf(input: UploadPdfInput): void {
  const maxSize = positiveIntegerEnv("MAX_SUBMISSION_FILE_SIZE_BYTES", 5 * 1024 * 1024);
  if (
...
export async function uploadPdf(input: UploadPdfInput): Promise<StoredObject> {
  ...
  try {
    ...
    throw new ApplicationError("EXTERNAL_PROVIDER_ERROR", 502, "Unable to store submission");
  }
}
