# S1-BE-03 — Participant, Attendance & Workshop Enrollment
...
- **Contract:** `POST /api/attendances`, `POST /api/workshops/enroll`, and the
  reusable Attendance-confirmation service consumed by BE-06
- **Owned files:** `services/participant.service.ts`, `services/attendance.service.ts`, `app/api/attendances/route.ts`, `app/api/workshops/enroll/route.ts`, entry-flow tests
...
- implement `enrollWorkshop(...)` without creating an Attendance;
- preserve an existing pending registration and return `409` for active;
- expose the participant service interface before BE-03B integrates Attendance.
...
- Attendance new/pending/verified and confirmation-service tests;
...
- implement `createAttendance` and `confirmAttendanceForVerifiedParticipant`;
- keep public Attendance creation `PENDING` and session-confirmed Attendance `VERIFIED`;
...
Attendance and same-email Attendance/workshop entry reuses one Participant. They agree on
the final PR together; any contract or shared-file change requires approval from
both PICs and Backend Lead.


## Goal (1 sentence)
Create/reuse one Participant from either public entry flow, create Attendance only for an explicit offline event registration, and never duplicate identity data.
...
- One participant has at most one optional Attendance and one optional workshop registration.
- `POST /api/attendances` owns explicit offline Attendance creation. Workshop enrollment
  must never create an Attendance.
...
- To keep this work order independently mergeable, BE-03 owns participant/Attendance
  persistence, two public entry routes, and reusable session-derived Attendance
...
```
For an existing `PENDING` Attendance, return `200 OK` with the existing ID/status
and no new automatic email. For an existing `VERIFIED` Attendance, return `409`.
...
### Attendance confirmation service for BE-06 route integration
```http
confirmAttendanceForVerifiedParticipant(participantId, classification)
```
BE-06 owns the `POST /api/attendances/confirm` Route Handler and passes the trusted
ID returned by `requireVerifiedParticipant`. Its request body contains only
`attendeeType` and, for a student, a non-empty `institution`.

- no Attendance -> create `VERIFIED` Attendance with that classification and return `201`;
- existing `PENDING` Attendance -> promote it to `VERIFIED` only when its saved
  classification exactly matches the request, then return `200`;
- existing `VERIFIED` Attendance -> `409`;
- a mismatched pending classification -> `409`;
...
seam with `purpose = WORKSHOP`. Do not create `Attendance`.
...
5. For `POST /api/attendances`, if the participant already has an Attendance:
   - if `VERIFIED`: return conflict (`409`) rather than create another Attendance;
   - if `PENDING`: return the existing Attendance ID as pending so resend flow can be used; do not create another Attendance row.
6. A new Attendance status is always `PENDING`; this route never creates
   `WorkshopRegistration`.
7. Do not write verification-token fields in this module.
8. `POST /api/attendances/confirm` requires the trusted verified session: create a
   `VERIFIED` Attendance if absent or promote a `PENDING` Attendance only with
   matching attendee classification; never accept identity.
9. `POST /api/workshops/enroll` uses the same create/reuse Participant logic,
   creates no Attendance, and creates one `PENDING` WorkshopRegistration with the
...
confirmAttendanceForVerifiedParticipant(
  ...
  input: { attendeeType: "STUDENT" | "PUBLIC"; institution?: string },
)
...
link using `participantId` + `purpose` without a second email lookup. Attendance
entry returns its `attendanceId`; workshop enrollment returns its registration status
...
3. Add Attendance create/get plus session-based Attendance confirmation logic.
4. Implement Attendance and workshop-enrollment routes with BE-02 schemas/error
   helpers, plus reusable Attendance-confirmation service behavior.
...
- BE-06 owns the `POST /api/attendances/confirm` Route Handler and calls
  `confirmAttendanceForVerifiedParticipant` with its trusted participant ID.
- BE-04/BE-05 may edit only the verification-dispatch call sites in
  `app/api/attendances/route.ts` and `app/api/workshops/enroll/route.ts` after this
  work order is approved.
- They may not rewrite participant/Attendance persistence rules, overwrite a pending
  workshop registration, or cause workshop enrollment to create an Attendance without
...
- Do not add workshop/submission fields to Participant or Attendance beyond the
  required attendance classification (`attendeeType` and optional `institution`).
- Do not accept arbitrary workshop path values; only `CTF`, `BCC`, and `CP`
  from the shared schema are valid.
- Do not move phone number or NIM onto Participant, Attendance, or a new identity
  table.
- Do not change Prisma schema or shared Zod/error contract.

## Done = (acceptance criteria — become tests)
- [ ] New email creates exactly one Participant + one `PENDING` Attendance.
- [ ] Workshop-only enrollment creates/reuses Participant, no Attendance, and exactly
...
- [ ] Attendance-confirmation service creates/promotes one `VERIFIED` Attendance from a
  trusted participant ID, accepts only matching classification fields, and accepts no identity fields.
- [ ] Same email with different casing reuses the same Participant.
- [ ] Repeated request for an existing `PENDING` Attendance does not create a second Attendance.
- [ ] Existing `VERIFIED` Attendance returns `409 Conflict`.
- [ ] Invalid request is rejected through shared validation (`400`).
- [ ] Route never writes/sends raw verification tokens.
- [ ] Tests include: Attendance new/pending/verified, workshop-only enrollment,
  repeat pending workshop enrollment, session-based Attendance confirmation, and
  invalid payload.
