...
function invalidPayload(): ApplicationError {
  return new ApplicationError("VALIDATION_ERROR", 400, "Invalid request payload");
}
...
async function parseSubmissionForm(request: Request): Promise<{
  ...
  if (
    [...form.keys()].some((key) => key !== "competitionPath" && key !== "file") ||
  ...
  const input = submissionSchema.parse({ competitionPath: form.get("competitionPath") });
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return successResponse(
      { id: submission.id, competitionPath: submission.competitionPath, fileName: submission.fileName },
