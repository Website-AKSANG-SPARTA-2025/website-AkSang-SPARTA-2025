...
afterEach(() => {
  vi.unstubAllGlobals();
  if (originalBaseUrl === undefined) delete process.env.AEGIS_VERIFICATION_BASE_URL;
  else process.env.AEGIS_VERIFICATION_BASE_URL = originalBaseUrl;
  if (originalApiKey === undefined) delete process.env.AEGIS_VERIFICATION_API_KEY;
...
describe("Aegis verification adapter", () => {
  it("posts an email to the configured endpoint with the server-only key", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("https://aegis.example.test/api/verification", {
      method: "POST",
  ...
  it("encodes the status email in the query and returns an active pending link", async () => {
    ...
    await expect(getAegisVerificationStatus("ada+event@example.com")).resolves.toEqual({
      status: "not_verified",
      registeredAt: "2026-08-11T00:00:00.000Z",
      linkActive: true,
      linkExpiresAt: "2026-08-11T01:00:00.000Z",
  ...
  it("returns verified and not-registered status responses", async () => {
    ...
    await expect(getAegisVerificationStatus("ada@example.com")).resolves.toEqual({
      status: "verified",
      verifiedAt: "2026-08-11T01:00:00.000Z",
    });
    await expect(getAegisVerificationStatus("ada@example.com")).resolves.toEqual({
  ...
  it("maps documented send errors without flattening their meaning", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse(429, { status: "error", error: "resend_too_soon", retryAfter: 41 }),
    );
    await expect(sendAegisVerification("ada@example.com")).rejects.toMatchObject({
      code: "RATE_LIMITED",
      status: 429,
  ...
  it("maps malformed or unreachable upstream responses to a safe service failure", async () => {
    ...
    await expect(getAegisVerificationStatus("ada@example.com")).rejects.toMatchObject({
      code: "EXTERNAL_PROVIDER_ERROR",
      status: 502,
    });
