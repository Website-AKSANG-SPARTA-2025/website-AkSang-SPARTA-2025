...
describe("verified participant resolution", () => {
  ...
  it("rejects absent and tampered sessions", async () => {
    ...
    const token = await createParticipantSession("participant-1");
    await expect(requireVerifiedParticipant(requestWithToken(`${token}x`))).rejects.toMatchObject({ status: 401 });
  });

  it("rejects missing or unverified participants", async () => {
    const token = await createParticipantSession("participant-1");
    await expect(requireVerifiedParticipant(requestWithToken(token))).rejects.toMatchObject({ status: 401 });
  ...
  it("returns the exact verified participant from the trusted session ID", async () => {
    const token = await createParticipantSession("participant-1");
    const verified = { id: "participant-1", emailVerifiedAt: new Date("2026-08-11T00:00:00.000Z") };
    participant = verified;

    await expect(requireVerifiedParticipant(requestWithToken(token))).resolves.toBe(verified);
    expect(findUnique).toHaveBeenCalledWith({ where: { id: "participant-1" } });
  });
});
