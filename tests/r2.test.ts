...
function input(overrides: Partial<{ bytes: Uint8Array; originalFileName: string; contentType: string }> = {}) {
  return {
    bytes: pdf,
...
afterEach(() => {
  ...
  if (originalEnv.maxSize === undefined) delete process.env.MAX_SUBMISSION_FILE_SIZE_BYTES;
...
describe("private PDF storage", () => {
  it("accepts only nonempty PDF bytes within the configured size", () => {
    ...
    expect(validationError(input({ bytes: new Uint8Array() }))).toMatchObject({ status: 400 });
    expect(validationError(input({ contentType: "text/plain" }))).toMatchObject({ status: 400 });
    expect(validationError(input({ bytes: new TextEncoder().encode("not a pdf") }))).toMatchObject({
      status: 400,
    });
    expect(validationError(input({ bytes: new Uint8Array(21).fill(1) }))).toMatchObject({ status: 400 });
  });
  ...
  it("uploads with a generated key and sanitized display filename", async () => {
    ...
    expect(stored).toMatchObject({ contentType: "application/pdf", size: pdf.byteLength });
