...
vi.mock("../lib/auth", () => ({ requireVerifiedParticipant: boundaries.requireVerifiedParticipant }));
vi.mock("../lib/r2", () => ({ validatePdf: boundaries.validatePdf }));
vi.mock("../services/submission.service", () => ({ createSubmission: boundaries.createSubmission }));
vi.mock("../services/workshop.service", () => ({
  findActiveRegistrationByParticipantId: boundaries.findActiveRegistrationByParticipantId,
}));
...
function submissionRequest(options: {
  ...
  form.set("file", options.file ?? new File([pdf], "solution.pdf", { type: "application/pdf" }));
  for (const [key, value] of Object.entries(options.extra ?? {})) form.set(key, value);

  return new Request("https://app.example.test/api/submissions", { method: "POST", body: form });
}

function guardedRequest(): { request: Request; formData: ReturnType<typeof vi.fn> } {
...
beforeEach(() => {
  ...
  boundaries.requireVerifiedParticipant.mockResolvedValue({ id: "participant-1" });
...
describe("submission route", () => {
  it("requires a verified session before parsing multipart data", async () => {
    ...
    expect(boundaries.findActiveRegistrationByParticipantId).not.toHaveBeenCalled();
  ...
  it("stops unregistered verified participants before parsing or upload", async () => {
    ...
    expect(body).toEqual({ success: false, message: "Participant is not registered for the workshop" });
    expect(formData).not.toHaveBeenCalled();
  ...
  it("rejects invalid paths and client-supplied identity fields before upload", async () => {
    const invalidPath = await POST(submissionRequest({ competitionPath: "OTHER" }));
    const forgedIdentity = await POST(submissionRequest({ extra: { email: "forged@example.test" } }));
  ...
  it("rejects an invalid PDF before it reaches the submission service", async () => {
    boundaries.validatePdf.mockImplementation(() => {
      throw new ApplicationError("VALIDATION_ERROR", 400, "Only PDF files are allowed");
    });

    const response = await POST(
      submissionRequest({ file: new File(["not a PDF"], "fake.pdf", { type: "application/pdf" }) }),
    );
    const body = await response.json();

    expect(response.status).toBe(400);
    expect(body).toEqual({ success: false, message: "Only PDF files are allowed" });
  ...
  it("creates a submission with only the trusted active registration", async () => {
    ...
    expect(boundaries.findActiveRegistrationByParticipantId).toHaveBeenCalledWith("participant-1");
  ...
  it("returns a safe error when persistence fails after upload cleanup", async () => {
    boundaries.createSubmission.mockRejectedValue(new Error("database unavailable"));
