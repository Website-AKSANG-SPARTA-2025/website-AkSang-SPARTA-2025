...
vi.mock("../services/attendance.service", () => ({
  confirmAttendanceForVerifiedParticipant: boundaries.confirmAttendanceForVerifiedParticipant,
}));
...
describe("verified Attendance confirmation route", () => {
  it("requires a verified session before trusting any confirmation", async () => {
    ...
    const response = await confirmRoute(jsonRequest({ attendeeType: "PUBLIC" }));

    expect(response.status).toBe(401);
    expect(boundaries.confirmAttendanceForVerifiedParticipant).not.toHaveBeenCalled();
  });
  ...
  it("rejects identity fields and incomplete student classification", async () => {
    ...
    const missingInstitution = await confirmRoute(jsonRequest({ attendeeType: "STUDENT" }));
  ...
  it("creates a verified Attendance using only the trusted participant ID", async () => {
    ...
    expect(boundaries.confirmAttendanceForVerifiedParticipant).toHaveBeenCalledWith("participant-1", {
      attendeeType: "PUBLIC",
    });
  ...
  it("returns 200 for a matching pending Attendance promotion and 409 for conflicts", async () => {
    ...
    const promoted = await confirmRoute(jsonRequest({ attendeeType: "PUBLIC" }));
    expect(promoted.status).toBe(200);

    boundaries.confirmAttendanceForVerifiedParticipant.mockRejectedValue(
      new ApplicationError("CONFLICT", 409, "Attendance classification does not match"),
    );
    const conflict = await confirmRoute(jsonRequest({ attendeeType: "PUBLIC" }));
