...
describe("submission service", () => {
  it("uploads then persists only the approved submission metadata", async () => {
    create.mockResolvedValue({ id: "submission-1", competitionPath: "CTF", fileName: "solution.pdf" });
    ...
    expect(result).toMatchObject({ id: "submission-1", competitionPath: "CTF", fileName: "solution.pdf" });
  ...
  it("deletes the uploaded key exactly once when metadata persistence fails", async () => {
    ...
    await expect(
      createSubmission({ workshopRegistrationId: "registration-1", competitionPath: "CTF", file: file() }),
    ).rejects.toThrow("database unavailable");

    expect(boundaries.deleteObject).toHaveBeenCalledTimes(1);
    expect(boundaries.deleteObject).toHaveBeenCalledWith("submissions/server-generated.pdf");
  });

  it("keeps the persistence error when orphan cleanup also fails", async () => {
    create.mockRejectedValue(new Error("database unavailable"));
    boundaries.deleteObject.mockRejectedValue(new TypeError("cleanup unavailable"));
    ...
    expect(errorLog).toHaveBeenCalledWith("Submission persistence cleanup failed", {
