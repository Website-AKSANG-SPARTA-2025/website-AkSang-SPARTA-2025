...
vi.mock("../services/workshop.service", () => ({ registerParticipant: boundaries.registerParticipant }));
...
describe("workshop activation route", () => {
  it("requires a verified session", async () => {
    ...
    const response = await POST(jsonRequest({ competitionPath: "CTF", phoneNumber: "+62812345678" }));
  ...
  it("rejects forged identity fields and invalid registration data", async () => {
    const forged = await POST(
      jsonRequest({ competitionPath: "CTF", phoneNumber: "+62812345678", name: "forged" }),
    );
  ...
  it("activates workshop registration with trusted identity only", async () => {
    boundaries.registerParticipant.mockResolvedValue({ id: "registration-1", competitionPath: "CTF" });
    ...
    expect(body).toEqual({
      ...
      data: { id: "registration-1", competitionPath: "CTF", invitationAvailable: true },
    });
    expect(boundaries.registerParticipant).toHaveBeenCalledWith("participant-1", {
      competitionPath: "CTF",
  ...
  it("maps an existing pending or active registration to conflict", async () => {
    boundaries.registerParticipant.mockRejectedValue(
      new ApplicationError("CONFLICT", 409, "Workshop participant already registered"),
    );
