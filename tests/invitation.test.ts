...
describe("protected workshop invitations", () => {
  ...
  it("returns 401 without exposing a community URL when the session is invalid", async () => {
    ...
    const response = await GET(new Request("https://app.example.test/api/workshops/invitation"));
  ...
  it("redirects only a verified participant with an active stored path", async () => {
    boundaries.findActiveRegistrationByParticipantId.mockResolvedValue({ competitionPath: "CTF" });

    const response = await GET(new Request("https://app.example.test/api/workshops/invitation"));

    expect(response.status).toBe(302);
    expect(response.headers.get("location")).toBe("https://chat.example.test/ctf");
  });
