...
beforeEach(() => {
  create = vi.fn();
  findFirst = vi.fn();
  boundaries.getPrisma.mockReturnValue({ workshopRegistration: { create, findFirst } });
});
...
describe("workshop activation service", () => {
  it("creates one ACTIVE registration from the trusted participant ID", async () => {
    ...
    await expect(
      registerParticipant("participant-1", { competitionPath: "CTF", phoneNumber: "+62812345678" }),
    ).resolves.toMatchObject({ status: "ACTIVE", competitionPath: "CTF", nim: null });
  ...
  it("returns only active registrations for downstream eligibility", async () => {
    ...
    await expect(findActiveRegistrationByParticipantId("participant-1")).resolves.toBeNull();
    expect(findFirst).toHaveBeenCalledWith({
      where: { participantId: "participant-1", status: "ACTIVE" },
    });

    const active = { id: "registration-1", participantId: "participant-1", status: "ACTIVE" };
    findFirst.mockResolvedValue(active);
    await expect(findActiveRegistrationByParticipantId("participant-1")).resolves.toBe(active);
  });
});
